...
namespace ETModel
{
	...
	public class KChannel : AChannel
	{
		...
		public void HandleConnnect(uint remoteConn)
		{
			...
			SetOutput();
            //工作模式：
            //nodelay : 是否设置数据后合并发送 0不启用；1启用
            //interval ：协议内部工作的间隔时间，单位毫秒，比如 10ms或者 20ms
            //resend ：快速重传模式，默认0关闭，可以设置2（2次ACK跨越将会直接重传）
            //nc ：是否关闭流控，默认是0代表不关闭，1代表关闭
            Kcp.KcpNodelay(this.kcp, 1, 10, 1, 1);
            //最大窗口：该调用将会设置协议的最大发送窗口和最大接收窗口大小
            Kcp.KcpWndsize(this.kcp, 256, 256);
        ...
        //发送接口 KcpSend or sendBuffer.Enqueue
